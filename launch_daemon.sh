#!/bin/bash
if [ -f "/root/dslv-zpdi/.debug_mode" ] || [ -f "$(dirname "${BASH_SOURCE[0]}")/.debug_mode" ]; then
    set -x
fi
...
echo "Initiating 2-second pause before resolving environment variables..."
sleep 2

PROOT_DISTRO=/data/data/com.termux/files/usr/bin/proot-distro
PROJECT_DIR=/root/dslv-zpdi

echo "Variables resolved. Buffering for 2 seconds before stopping existing processes..."
sleep 2
...
echo "Checking for existing data locks... buffering 2s..."
sleep 2
...
echo "[*] Starting zpdi supervisor (independent proot session)... buffering 3s..."
sleep 3
