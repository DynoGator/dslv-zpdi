...
class RFAnomalyPanel:
    ...
    def __init__(self, waterfall, pipeline=None, border_style: str = "bright_yellow"):
    ...
    def render(self, compact: bool = False) -> Panel:
        ...
        r_global = 0.0
        if self.pipeline:
            # Try to get latest coherence from pipeline cache
            health = getattr(self.pipeline, "_health_cache", None)
            if health:
                h_data = health.val or {}
                coh = h_data.get("coherence", {})
                r_smooth = coh.get("r_smooth", 0.0)
                r_global = coh.get("r_global", 0.0)

        if compact:
            # Row 1: Peak + Freq
            t.add_row("Pk", f"[{_dbm_style(peak)}]{peak:5.1f}dBm[/] [dim]•[/] {_fmt_freq(freq).strip()}")
            # Row 2: SNR + Coh
            coh_sty = "bright_green" if r_smooth >= 0.4 else "yellow" if r_smooth >= 0.15 else "dim"
            t.add_row("SNR", f"[{_snr_style(snr)}]{snr:4.1f}dB[/] [dim]•[/] [{coh_sty}]r{r_smooth:.2f}[/]")
            # Row 3: Hit + Source
            anom_sty = "bright_red" if anomalies >= 5 else "bright_yellow" if anomalies >= 1 else "bright_green"
            t.add_row("Hit", f"[{anom_sty}]{anomalies} bins[/] [dim]•[/] {source[:8]}")
        else:
            # Peak now
            peak_line = Text()
            ...
            coh_line.append(f"r_smooth {r_smooth:.4f}", style=coh_sty)
            coh_line.append("  R_global ", style="dim")
            coh_line.append(f"{r_global:.4f}", style="bright_magenta")
            t.add_row("Coherence", coh_line)
