...
# ==============================================================================
# Do NOT force real SDR data ingestion on boot for the TUI (WaterfallPanel).
# The background dslv-zpdi.service requires exclusive hardware buffer access
# to the SDR via libiio. If the TUI also attempts to connect directly, they
# will collide and crash the pipeline. The TUI will still show real data by 
# parsing the JSON telemetry outputs generated by the background pipeline.
export DSLV_DASHBOARD_REAL_SDR=0
