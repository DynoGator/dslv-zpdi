...
# - Small HDMI displays (800x480): compact terminals that actually fit the screen.
# - 10" Lenovo HDMI touchscreen (1280x800): single large terminal using the two-column layout.
...
if [ "${SCREEN_W:-0}" -le 1024 ] && [ "${SCREEN_H:-0}" -le 600 ] \
   && [ "${SCREEN_W:-0}" -gt 0 ]; then
    # Small screens: single fullscreen window, compact layout, no second terminal.
    GEO_MAIN="92x30+0+0"
    GEO_WF="92x30+0+0"
    COMPACT_MODE=1
    TEN_INCH_MODE=0
...
if [ "$COMPACT_MODE" = "1" ]; then
    # Compact screen: single fullscreen window — the main dashboard embeds the
    # waterfall panel, so no separate waterfall window is needed.
    # Spawning two windows on an 800x480 screen just fights for space.
    SAY "  - opening single dashboard window (Compact mode) ($TERMCMD)"
    case "$TERMCMD" in
        lxterminal)
