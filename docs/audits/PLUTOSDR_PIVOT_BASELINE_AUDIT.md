...
│   ├── hal_factory.py             # get_hal() — auto/PlutoSDRplus/pluto/sim
│   ├── hal_hardware.py            # PlutoSDRplus-centric HardwareHAL
...
|---------|------|--------|----------|
| `HardwareHAL` | `hal_hardware.py` | SoapySDR (`driver=PlutoSDRplus`) + pyPlutoSDRplus | **Tightly PlutoSDRplus-coupled** despite generic name |
| `PlutoHAL` | `hal_pluto.py` (untracked WIP) | libiio `iio` + SoapySDR Pluto | Moderate; clones `HardwareHAL` timing logic |
| `SimulatedHAL` | `hal_simulated.py` | None | Deterministic GPSDO + PlutoSDRplus simulator |

### PlutoSDRplus coupling details

- `hal_hardware.py` hardcodes `driver="PlutoSDRplus"` in SoapySDR path.
- pyPlutoSDRplus fallback sets PlutoSDRplus-specific LNA/VGA gains and amp lockout.
- `verify_gpsdo_lock()` calls `PlutoSDRplus_debug --clock_source`.
- `provision_tier1.py` and dashboard `hardware.py` are PlutoSDRplus/LBE-1421-centric.
- `pyPlutoSDRplus>=0.2.0` is a **mandatory** dependency in `pyproject.toml`.
- Auto-detection order in `get_hal()` is PlutoSDRplus → Pluto → simulator.
...
| `GPSDOLockMonitor` | NMEA + chronyc RMS + `PlutoSDRplus_debug` | Not wired into production pipeline; PlutoSDRplus-only |
...
File: `install_dslv_zpdi.sh` (Rev 5.0.1-LBE1421-RESCUE)

- `TIER1_PACKAGES` includes `PlutoSDRplus`, `libPlutoSDRplus-dev`, `soapysdr-module-PlutoSDRplus`, `python3-soapysdr`.
- No separate `--tier1-pluto` / `--PlutoSDRplus-legacy` / `--simulator` install modes.
...
- `dslv-zpdi-PlutoSDRplus-init.service` (PlutoSDRplus-specific)
...
main_pipeline.py / capture_baseline.py
    └── get_hal(...) [hal_factory.py]
            ├── HardwareHAL [hal_hardware.py] ── SoapySDR/pyPlutoSDRplus
...
- `HardwareHAL` is effectively a PlutoSDRplus HAL with a generic name.
...
- `PHASE_2A_TIER_1_BUILD_SHEET.md` still calls the PlutoSDRplus the Tier-1 SDR.
- `README.md` and release notes reference PlutoSDRplus as canonical Tier-1.
- `CHANGELOG.md` and turnover documents describe the LBE-1421 + PlutoSDRplus build.

### Tests tied to PlutoSDRplus

- `tests/test_hardware_failure_paths.py` — mocks `pyPlutoSDRplus` and SoapySDR PlutoSDRplus paths.
- `tests/test_timing_monitor.py` — tests `GPSDOLockMonitor` (PlutoSDRplus-only helper).
- No custom `@pytest.mark.hardware` or `@pytest.mark.soak` markers.

### Installer coupling

- `install_dslv_zpdi.sh` installs PlutoSDRplus packages unconditionally in Tier-1 mode.
- `Dockerfile` installs `libPlutoSDRplus-dev`, `soapysdr-module-PlutoSDRplus`.

### Runtime coupling

- `hal_factory.py` imports `hal_hardware.py` eagerly, which eagerly imports `SoapySDR` and `PlutoSDRplus`.
- `pyPlutoSDRplus` monkey-patching happens at module load.
...
- Dashboard `hardware.py` probes `PlutoSDRplus_info` and `iio.Context` but is visually PlutoSDRplus-first.
- Dashboard `waterfall.py` supports SIM/PlutoSDRplus/PLUTO sources.
...
2. Move PlutoSDRplus-specific code to `sdr/PlutoSDRplus_legacy.py` and make it optional.
...
10. Restructure installer with `--tier1-pluto`, `--PlutoSDRplus-legacy`, `--simulator` modes.
