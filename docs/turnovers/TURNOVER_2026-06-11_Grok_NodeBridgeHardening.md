...
- Starting commit: 9462a2b (Rev 5.0.0 merge on main)
...
## Work Completed
- **Task A (critical blocker fix):** Broadened all native-library import guards (`except (ImportError, OSError)`) in hal_hardware.py (Soapy + pyPlutoSDRplus with full Rev 5.0.x + SPEC-005A.HAL-HW commentary), hdf5_writer.py, radon_session_writer.py, radoneye_ingestor.py (bleak including bare from sites). Pure-Python guards (flask, inner pyserial) left as-is with justification. Root cause: PlutoSDRplus CDLL at import time raises OSError on no-.so hosts → collection of 2 test modules failed (0 tests collected). After: 113 passed.
...
- Version bump to 4.8.1 + full synchronization (pyproject, __init__, README, CHANGELOG, new RELEASE_NOTES_v5.0.0.md). All commits atomic, full §2 contract green before every commit + after push.
...
Result (this session final): version sync 4.8.1, orphan OK, repo guard OK, ruff clean, 113 passed, test_pipeline "Rev 5.0.1 Tests" + ALL 10 PASS.
...
## Risks / Notes
- This host has no PlutoSDRplus/libPlutoSDRplus — all work validated under DEV_SIMULATOR=1 only. Do not treat simulator numbers as hardware-truth evidence.
...
## Next Actions
- **Highest priority:** P1 Hardware Truth Path on the Pi 5 (LBE-1421 + PlutoSDRplus + PPS + real NMEA). Capture evidence only into docs/validation-logs/ after confirming clean.
