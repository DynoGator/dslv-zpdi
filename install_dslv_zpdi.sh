...
# Emit consolidated operation list in required format (new for review robustness)
echo "=== INSTALLATION OPERATION SUMMARY (main Tier-1 installer) ==="
echo "SUCCEEDED OPERATIONS (from log_ok / successful soft steps above):"
echo "  - OS detection and Python >=3.9 check"
echo "  - Repository clone/update + pyproject.toml validation"
echo "  - venv creation + pip upgrade"
echo "  - pip install -e '.[dev]' (or fallback -e .)  [incorporates all new pyproject deps + src/dslv_zpdi layout]"
echo "  - pip check (advisory)"
echo "  - dslv_zpdi.main_pipeline import smoke test"
echo "  - (plus any --harden/--dashboard etc steps that succeeded via log_ok)"
echo ""
echo "FAILED OPERATIONS (if any logged above as [DSLV-ERR] or soft warn treated as fail):"
echo "  - See individual [DSLV-ERR] or [DSLV-WARN] lines for details."
echo "  Recommended corrective action per failure: Re-run with --skip-apt or --simulator; check the specific log message (e.g. missing libhackrf-dev -> apt install; permission -> run with sudo where noted; python version -> upgrade python3)."
echo "  For editable install fails: ensure pyproject.toml present, python3-dev + build-essential, then 'cd \$INSTALL_DIR && source venv/bin/activate && pip install -e .[dev]' "
echo "================================================================"
