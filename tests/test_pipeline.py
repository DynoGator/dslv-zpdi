...
def test_serialization_roundtrip():
    p = IngestionPayload(node_id="C", sensor_id="S", modality=SensorModality.RF_SDR.value, payload_uuid=str(uuid.uuid4()), timestamp_utc=time.time(), gps_locked=True, extracted_phases=[0.1])
    d = json.loads(p.to_json())
    assert SensorModality(d['modality']) == SensorModality.RF_SDR
    print("  TEST 2 PASS: Serialization ✅")

def test_state_machine():
    p = IngestionPayload(node_id="C", sensor_id="S", modality=SensorModality.RF_SDR.value, payload_uuid=str(uuid.uuid4()), timestamp_utc=time.time(), gps_locked=False)
...
def test_attestation():
    ...
    assert hmac.new(b"key", tp, hashlib.sha256).hexdigest() == hmac.new(w.key, tp, hashlib.sha256).hexdigest()
