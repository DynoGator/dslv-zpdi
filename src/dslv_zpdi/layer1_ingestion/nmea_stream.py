...
Broad exception policy (justified here):
- Thread entry points and I/O loops use broad "except Exception" (with pylint disable)
  to guarantee the reader thread (and thus the whole daemon) never dies on
  transient hardware or serial errors.
- All such cases MUST log at ERROR (or higher) and attempt recovery/backoff.
- This pattern is acceptable only for "never-crash" background services.
  See also: HealthReporter, MVIP6Watchdog, and thread resilience notes in
  specs/SPEC-011* and docs/collaboration/.
"""
    ...
    def _run(self) -> None:
        while not self._stop.is_set():
            ...
            try:
                ...
                # Justified broad except: long-running GPSDO reader thread must never
                # crash the parent daemon on transient USB/serial glitches.
